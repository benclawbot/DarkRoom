...
async function makeThumbnail(blob,max=520){try{const image=await decodeImageElement(blob),scale=Math.min(1,max/Math.max(image.naturalWidth,image.naturalHeight)),c=document.createElement('canvas');c.width=Math.max(1,Math.round(image.naturalWidth*scale));c.height=Math.max(1,Math.round(image.naturalHeight*scale));const x=c.getContext('2d');x.imageSmoothingEnabled=true;x.imageSmoothingQuality='high';x.drawImage(image,0,0,c.width,c.height);releaseDecodedImage(image);return await new Promise(r=>c.toBlob(r,'image/jpeg',.82))}catch{return null}}
