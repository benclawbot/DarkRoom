...
const normalizeEditNumber=(value,fallback)=>{if(value===null||value===undefined||value==='')return fallback;const n=Number(value);return Number.isFinite(n)?n:fallback};
function normalizeCurvePoints(points){if(!Array.isArray(points))return[];const clean=[];for(const point of points){const x=normalizeEditNumber(point?.x,NaN),y=normalizeEditNumber(point?.y,NaN);if(!Number.isFinite(x)||!Number.isFinite(y))continue;clean.push({x:Math.max(0,Math.min(1,x)),y:Math.max(0,Math.min(1,y))})}clean.sort((a,b)=>a.x-b.x);const dedup=[];for(const point of clean){const previous=dedup.at(-1);if(previous&&Math.abs(previous.x-point.x)<.002)previous.y=point.y;else dedup.push(point)}if(dedup.length<2)return[];if(dedup[0].x>.001)dedup.unshift({x:0,y:dedup[0].y});else dedup[0].x=0;if(dedup.at(-1).x<.999)dedup.push({x:1,y:dedup.at(-1).y});else dedup.at(-1).x=1;return dedup}
function normalizeEdits(edits){const base=defaultEdits(),input=edits&&typeof edits==='object'?edits:{};for(const key of Object.keys(base)){if(key==='curvePoints'||typeof base[key]==='boolean'||typeof base[key]==='string')continue;base[key]=normalizeEditNumber(input[key],base[key])}base.flipX=input.flipX===true||input.flipX==='true';base.flipY=input.flipY===true||input.flipY==='true';base.cropAspect=typeof input.cropAspect==='string'?input.cropAspect:base.cropAspect;base.useAISource=input.useAISource===true||input.useAISource==='true';base.curvePoints=normalizeCurvePoints(input.curvePoints);return base}
...
function normalizePhoto(p){p.edits=normalizeEdits(p.edits);p.albumIds=p.albumIds||[];p.rating=p.rating||0;p.flag=p.flag||'none';p.colorLabel=p.colorLabel||'none';p.adjustmentLayers=(p.adjustmentLayers||[]).map(l=>({...l,enabled:l.enabled!==false,opacity:normalizeEditNumber(l.opacity,1),blend:l.blend||'normal',edits:normalizeEdits(l.edits)}));p.imageLayers=(p.imageLayers||[]).map(l=>({...l,enabled:l.enabled!==false,opacity:normalizeEditNumber(l.opacity,1),blend:l.blend||'normal',scale:normalizeEditNumber(l.scale,100),x:normalizeEditNumber(l.x,0),y:normalizeEditNumber(l.y,0),rotation:normalizeEditNumber(l.rotation,0)}));p.caption=p.caption||'';p.keywords=Array.isArray(p.keywords)?p.keywords:[];p.lut=p.lut||null;p.skyReplacementId=p.skyReplacementId||null;p.analysis=p.analysis||null;p.localEdits=(p.localEdits||[]).map(m=>({...m,enabled:m.enabled!==false,opacity:normalizeEditNumber(m.opacity,1),density:normalizeEditNumber(m.density,1),feather:normalizeEditNumber(m.feather,.6),flow:normalizeEditNumber(m.flow,.65),size:normalizeEditNumber(m.size,.08),adjust:{...localDefaultAdjust(),...(m.adjust||{})},strokes:m.strokes||[],modifiers:(m.modifiers||[]).map(x=>({...x,strokes:x.strokes||[]}))}));p.healOps=(p.healOps||[]).map(m=>({...m,enabled:m.enabled!==false,feather:normalizeEditNumber(m.feather,.55),flow:normalizeEditNumber(m.flow,1),size:normalizeEditNumber(m.size,.06),strokes:m.strokes||[]}));return p}
...
const decodedImageDimensions=image=>({width:image?.naturalWidth||image?.width||0,height:image?.naturalHeight||image?.height||0});
async function makeThumbnail(blob,max=520){try{const image=await decodeImageElement(blob),{width,height}=decodedImageDimensions(image),scale=Math.min(1,max/Math.max(width,height)),c=document.createElement('canvas');c.width=Math.max(1,Math.round(width*scale));c.height=Math.max(1,Math.round(height*scale));const x=c.getContext('2d');x.imageSmoothingEnabled=true;x.imageSmoothingQuality='high';x.drawImage(image,0,0,c.width,c.height);releaseDecodedImage(image);return await new Promise(r=>c.toBlob(r,'image/jpeg',.82))}catch{return null}}
async function fileDimensions(blob){const image=await decodeImageElement(blob),dimensions=decodedImageDimensions(image);releaseDecodedImage(image);return dimensions}
async function importFiles(files){const imageExts=new Set(['jpg','jpeg','png','webp','gif','bmp','avif','svg']);const candidates=[...files].filter(f=>{const type=typeof f.type==='string'?f.type.toLowerCase():'';const ext=String(f.name||'').toLowerCase().split('.').pop();return type.startsWith('image/')||imageExts.has(ext)||DarkRoomRAW?.isRaw?.(f)});if(!candidates.length)return toast('No supported images selected (JPG, PNG, WebP, AVIF, or RAW)');toast(`Importing ${candidates.length} photo${candidates.length>1?'s':''}…`);let imported=0;for(const f of candidates){try{let blob=f,width=0,height=0,raw=false,rawMeta=null,originalBlob=null;if(DarkRoomRAW?.isRaw?.(f)){toast(`Decoding RAW: ${f.name}`);const d=await DarkRoomRAW.decode(f);blob=d.blob;width=d.width;height=d.height;raw=true;rawMeta=d.metadata;originalBlob=f}else{const d=await fileDimensions(f);width=d.width;height=d.height}const thumbnailBlob=await makeThumbnail(blob);await put('photos',{id:uid(),name:f.name,type:f.type||blob.type||'image/raw',size:f.size,created:Date.now()+imported,modified:f.lastModified||Date.now(),width,height,blob,thumbnailBlob,originalBlob,raw,rawMeta,edits:defaultEdits(),adjustmentLayers:[],imageLayers:[],localEdits:[],healOps:[],caption:'',keywords:[],favorite:false,rating:0,flag:'none',albumIds:[]});imported++}catch(e){console.error('Import failed',f.name,e);toast(`Could not import ${f.name}`)}}await refreshData();toast(`${imported} photo${imported===1?'':'s'} added`)}
async function loadDemoPhotos(){const assets=['mountain-sunset.jpg','white-dunes.jpg','race-car.jpg'];toast('Loading local sample set…');try{const files=[];for(const name of assets){const response=await fetch('./docs/assets/examples/'+name);if(!response.ok)throw new Error('Could not load '+name);files.push(new File([await response.blob()],name,{type:'image/jpeg'}))}await importFiles(files)}catch(error){console.error('Demo import failed',error);toast('Sample set unavailable — add photos from your device')}}
