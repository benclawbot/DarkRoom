...
const context={
  ...
  caches:{
    ...
    keys:async()=>['darkroom-v21','darkroom-v22'],
...
assert.equal(opened[0],'darkroom-v22','the upgraded worker must populate a new cache generation');

let activatePromise;
listeners.activate({waitUntil:promise=>{activatePromise=promise}});
await activatePromise;
assert(deleted.includes('darkroom-v21'),'activation must remove the stale v21 app shell');
assert(!deleted.includes('darkroom-v22'),'activation must retain the current app shell');
