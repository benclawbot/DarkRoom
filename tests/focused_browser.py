...
def main():
    ...
    with serve() as url, sync_playwright() as p:
        ...
        assert page.locator('#editorOpenPhoto').is_visible()
        page.set_input_files('#fileInput',{'name':'mobile.png','mimeType':'image/png','buffer':sample_png()});page.wait_for_function("currentPhoto && document.querySelector('#editorCanvas').width>20",timeout=15000);page.locator('#editorEmptyPicker').wait_for(state='hidden',timeout=15000)
        assert page.locator('#panelToggle').count()==0
        assert page.locator('#sessionFilmstrip').count()==0
